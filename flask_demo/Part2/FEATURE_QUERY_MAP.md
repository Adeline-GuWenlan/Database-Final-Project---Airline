...
| Register airline staff | `AirlineStaff`, `StaffPermission` | `INSERT` staff row plus one or more permission rows inside a transaction |
...
| Book a flight | `Flight`, `SeatClass`, `Ticket`, `Purchases` | Transaction with `SELECT ... FOR UPDATE` for flight and seat-class rows, seat-count check, `INSERT` ticket, then `INSERT` purchase |
...
| Book on behalf of a customer | `AuthorizedBy`, `Customer`, `Flight`, `SeatClass`, `Ticket`, `Purchases` | Transaction that verifies agent-airline authorization, validates customer email, locks inventory rows, and inserts ticket + purchase |
...
| `enforce_ticket_capacity` | `Ticket` | Reject oversold seat-class inserts even if an application-side check is missed |
